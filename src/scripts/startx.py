...
"""
    ...
    # fixed 1 FPS on glxgears by disabling DPMS 
    # https://groups.google.com/g/virtualgl-users/c/d8k1ujwKJXo/m/EcQEI2-1CwAJ
    xorg_conf.append("""
Section "ServerFlags"
    Option "BlankTime" "0"
...
def startx(display, gpu_id):
    ...
    selected_device = devices[gpu_id:gpu_id+1]
    print("Selected device: {}".format(selected_device))
    try:
        fd, path = tempfile.mkstemp()
        with open(path, "w") as f:
            f.write(generate_xorg_conf(selected_device))
...
if __name__ == '__main__':
    ...
    print("Starting X on DISPLAY=:%s using GPU %s" % (args.display, args.gpu_id))
    startx(args.display, args.gpu_id)
